...
// ================================
// Mini-cart helper
// Tries every known Wix cart API in order; falls back to navigating to /cart.
// ================================
...
// Dynamic pages can use different dataset IDs — try all known candidates.
// ================================
...
// Import910 holds Cyth-specific fields (model, descriptions, lifecycle, support info).
// Import912 holds DigiKey/Newark pricing and stock.
// ================================
...
function parseDocDisplayName(wixUrl, model) {
  ...
  try {
    // wix:document://v1/<hash>/<encoded-filename>
    const parts = wixUrl.split("/");
    const encoded = parts[parts.length - 1] ?? "";
    const filename = decodeURIComponent(encoded).replace(/\.pdf$/i, "");
    // Strip leading <model>_ or <sku>_ prefix to isolate doctype
    const underscoreIdx = filename.indexOf("_");
    let doctype = underscoreIdx !== -1 ? filename.slice(underscoreIdx + 1) : filename;

    // If no underscore, filename may use spaces: "cDAQ-9178 Specifications"
    // Strip model prefix (case-insensitive) so we don't double it
...
// NOTE: productId param is unused — query is keyed on extData.sku (set by setPageContentAndStates).
...
//set engineering support text
  ...
  // Use simplified "noModel" layout if any of the three key fields are missing
  const isNoModel =
    extData.model === "empty" ||
